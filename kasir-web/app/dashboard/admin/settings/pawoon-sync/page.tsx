...
export default function PawoonSyncPage() {
  ...
  function extractItemDetails(mItem: any) {
    const name = mappings.itemName ? String(getValueByPath(mItem, mappings.itemName) || '').trim() : ''
    const sku = mappings.itemSku ? String(getValueByPath(mItem, mappings.itemSku) || '').trim() : ''
    const qty = mappings.itemQty ? parseExcelNumber(getValueByPath(mItem, mappings.itemQty)) : 0
    const price = mappings.itemPrice ? parseExcelNumber(getValueByPath(mItem, mappings.itemPrice)) : 0
    const discount = mappings.itemDiscount ? parseExcelNumber(getValueByPath(mItem, mappings.itemDiscount)) : 0
  ...
  async function handleStartSync() {
    ...
    if (syncMode === 'products') {
      const currentCategories = await loadCategories(selectedStoreId)

      for (let i = 0; i < productsArray.length; i++) {
        const item = productsArray[i]
        const productName = String(getValueByPath(item, mappings.name) || '').trim()
      ...
      try {
        ...
        for (let i = 0; i < productsArray.length; i++) {
          const item = productsArray[i]
          const sku = mappings.sku ? String(getValueByPath(item, mappings.sku) || '').trim() : ''
          const name = mappings.name ? String(getValueByPath(item, mappings.name) || '') : ''
          const rawStock = mappings.stock ? getValueByPath(item, mappings.stock) : 0
      ...
      try {
        ...
        for (let i = 0; i < targetTransactions.length; i++) {
          ...
          if (cashierName) {
            const matchedCashier = currentCashiers.find(c => c.name.toLowerCase() === cashierName.toLowerCase())
            if (matchedCashier) {
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-600 font-semibold">
                      {(() => {
                        ...
                        return previewList.slice(0, 5).map((item, idx) => {
                          ...
                          const name = String(getValueByPath(item, mappings.name) || '-')
                          const sku = String(getValueByPath(item, mappings.sku) || '')
                          const barcode = String(getValueByPath(item, mappings.barcode) || '')
                          const costPrice = Math.max(0, parseExcelNumber(getValueByPath(item, mappings.costPrice)))
                          const sellingPrice = Math.max(0, parseExcelNumber(getValueByPath(item, mappings.sellingPrice)))
                          const stock = Math.max(0, parseExcelNumber(getValueByPath(item, mappings.stock)))
                          const category = String(getValueByPath(item, mappings.category) || 'Umum')
