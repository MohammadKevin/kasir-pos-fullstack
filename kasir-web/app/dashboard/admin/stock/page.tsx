  ...
  const parsedDbOpnames = useMemo(() => {
    ...
    opnameMovements.forEach(m => {
      ...
      const groupKey = Object.keys(groups).find(key => {
        ...
        const sameTime = Math.abs(groupTimeMs - timeMs) < 30000 
  ...
  const combinedOpnameHistory = useMemo(() => {
    ...
    legacySubmitted.forEach(legacy => {
      ...
      const isDuplicated = allSubmitted.some(db => 
        ...
        Math.abs(new Date(db.createdAt).getTime() - new Date(legacy.createdAt).getTime()) < 60000 
      )
      if (!isDuplicated) {
      ...
      <html>
        ...
        <body>
          <div class="header">
            ...
            <p>NovaPOS Cloud &bull; Outlet/Cabang: \${storeName}</p>
          </div>
          <div class="meta-info">
